...
/**
 * spec/CoreIdeaExtractor.md §12.9 (N-N) — tách từ CoreIdeaExtractor. 1 bộ tiêu chí có thể áp dụng
 * cho nhiều category (bảng nối content_foundation_categories), nhưng 1 category chỉ dùng ĐÚNG 1 bộ
 * tại 1 thời điểm (unique(post_category_id) ở bảng nối). `$otherCategoryIds` là tập ĐẦY ĐỦ các
 * category KHÁC (ngoài `$category`) mà bộ tiêu chí này sẽ áp dụng chung sau khi lưu — thay thế
 * hoàn toàn tập cũ (sync semantics), category nào đang dùng bộ khác sẽ bị "chuyển nhà" sang bộ
 * này. `created_by` CHỈ set ở lần tạo đầu tiên (không ghi đè khi update lần sau bởi người khác).
 */
class UpsertCategoryFoundationAction
{
    ...
    public function handle(PostCategory $category, CategoryFoundationData $data, array $otherCategoryIds, int $userId): CategoryContentFoundation
    {
        return DB::transaction(function () use ($category, $data, $otherCategoryIds, $userId) {
            ...
            DB::table('content_foundation_categories')
            ...
            // TẮT hoạt động không "ma" xuất hiện trong `shared_with` trả về.
