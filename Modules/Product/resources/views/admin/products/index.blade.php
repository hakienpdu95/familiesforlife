...
<div x-data="productListPage({{ Js::from([
    'apiUrl' => route('backend.api.products.items'),
]) }})">
            ...
            <div class="flex flex-wrap gap-3 items-end">
                ...
                <div class="form-control flex-1 min-w-52">
                    <label class="label py-0.5">
                        ...
                        <span class="label-text-alt text-xs text-base-content/40">Tên, SKU</span>
                    </label>
                    <div class="input input-sm input-bordered flex items-center gap-2 bg-base-100">
                        ...
                        <input type="text" x-model="filters.search" @input.debounce.350ms="onFilterChange()"
                               placeholder="Nhập tên/SKU..." class="grow bg-transparent outline-none text-sm">
...
<dialog id="productDeleteModal" class="modal">
    <div class="modal-box max-w-sm">
        <h3 class="font-bold text-lg text-error">Xác nhận xoá</h3>
        <p class="py-3 text-sm text-base-content/70">
            Bạn có chắc muốn xoá sản phẩm
            <strong id="productDeleteItemName" class="text-base-content"></strong>?
        </p>
        <div class="modal-action mt-4">
            <button id="productConfirmDeleteBtn" class="btn btn-error btn-sm">Xoá</button>
            <button class="btn btn-ghost btn-sm" onclick="productDeleteModal.close()">Hủy</button>
        </div>
    </div>
