 ...
 * Alpine component + Tabulator cho dashboard/products — cùng pattern
...
const COLUMNS = [
    {
        ...
        formatter(cell) {
            ...
            if (d.sku) html += '<div class="text-xs text-base-content/40 font-mono">' + esc(d.sku) + '</div>';
            return html;
        },
    },
    {
        title: 'Danh mục', field: 'category', minWidth: 150, headerSort: false,
        formatter(cell) {
            return esc(cell.getValue()) || '<span class="text-base-content/25 text-xs">—</span>';
        },
    },
    {
        title: 'Loại', field: 'type_label', width: 130, headerSort: false,
    },
    {
        title: 'Giá', field: 'display_price', width: 130, hozAlign: 'right', headerSort: false,
        formatter(cell) {
            return esc(cell.getValue()) || '<span class="text-base-content/25 text-xs">—</span>';
        },
    },
    ...
    {
        title: 'Duyệt nội dung', field: 'approval_label', width: 140, hozAlign: 'center', headerSort: false,
        formatter(cell) {
            const d = cell.getRow().getData();
            if (!d.approval_label) return '<span class="text-base-content/30 text-xs">—</span>';
            return '<span class="badge badge-sm ' + esc(d.approval_badge) + '" title="Trạng thái duyệt nội dung — độc lập với trạng thái kinh doanh">' + esc(d.approval_label) + '</span>';
        },
    },
    {
        title: 'Bài viết dùng', field: 'used_in_articles_count', width: 120, hozAlign: 'center', sorter: 'number',
        formatter(cell) {
            const v = cell.getValue();
            return v > 0 ? '<span class="badge badge-sm badge-info">' + v + '</span>' : '<span class="text-base-content/30">0</span>';
        },
    },
    {
...
window.productDeleteConfirm = function (url, name) {
    ...
    const nameEl = document.getElementById('productDeleteItemName');
    if (nameEl) nameEl.textContent = '"' + name + '"';
    document.getElementById('productDeleteModal')?.showModal();
};

document.addEventListener('DOMContentLoaded', () => {
    const confirmBtn = document.getElementById('productConfirmDeleteBtn');
    ...
    confirmBtn.addEventListener('click', async function () {
        ...
        try {
            ...
            if (res.ok) {
                document.getElementById('productDeleteModal')?.close();
