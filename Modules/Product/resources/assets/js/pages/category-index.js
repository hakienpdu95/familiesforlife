 ...
 * Alpine component + Tabulator (dataTree) cho dashboard/products/categories — cùng pattern
 * Modules/Post/resources/assets/js/pages/category-index.js: không phân trang remote, cây danh
 * mục luôn tải toàn bộ 1 lượt (CategoryApiController trả cây lồng nhau qua field "children",
 * hoặc danh sách phẳng khi có tìm kiếm).
 *
 * Server data truyền vào qua x-data="categoryListPage({{ Js::from([...]) }})".
 */
...
const COLUMNS = [
    ...
    {
        title: 'Số sản phẩm', field: 'products_count', width: 120, hozAlign: 'center', sorter: 'number',
    },
    {
        ...
        formatter(cell) {
            ...
            if (d.can_delete) {
                html += '<button class="btn btn-ghost btn-xs btn-square text-error" title="Xoá"'
                    ...
                    + ' onclick="window.productCategoryDeleteConfirm(this.dataset.url, this.dataset.name)">'
...
window.productCategoryDeleteConfirm = function (url, name) {
    pendingDeleteUrl = url;
    const nameEl = document.getElementById('productCategoryDeleteItemName');
    if (nameEl) nameEl.textContent = '"' + name + '"';
    document.getElementById('productCategoryDeleteModal')?.showModal();
};

document.addEventListener('DOMContentLoaded', () => {
    const confirmBtn = document.getElementById('productCategoryConfirmDeleteBtn');
    ...
    confirmBtn.addEventListener('click', async function () {
        ...
        try {
            ...
            if (res.ok) {
                document.getElementById('productCategoryDeleteModal')?.close();
                window.productCategoryTable?.replaceData();
            } else {
                alert(data.message || 'Xoá thất bại. Vui lòng thử lại.');
            }
        } catch (e) {
            console.error('[product-category] delete failed', e);
    ...
    Alpine.data('categoryListPage', (serverData = {}) => {
        ...
        return {
            ...
            _setup() {
                ...
                tableInst = new window.Tabulator('#product-category-table', {
                    ajaxURL:    apiUrl,
                    ...
                    ajaxError: (error) => console.error('[product-category] API error', error),
                ...
                window.productCategoryTable = tableInst;
            },
