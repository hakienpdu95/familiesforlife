 ...
 * Alpine component + Tabulator cho dashboard/ai-video-studio — cùng pattern
 * Modules/Post/resources/assets/js/pages/article-index.js: remote pagination/sort/filter qua API,
 * xoá qua AJAX + modal xác nhận.
 *
 * Server data truyền vào qua x-data="projectListPage({{ Js::from([...]) }})".
...
const COLUMNS = [
    {
        title: 'Project', field: 'name', minWidth: 260, sorter: 'string', frozen: true,
        formatter(cell) {
            const d = cell.getRow().getData();
            let html = '<a href="' + esc(d.show_url) + '" class="font-medium text-sm link link-hover">' + esc(d.name) + '</a>';
            if (d.description) {
                html += '<p class="text-xs text-base-content/40 truncate max-w-xs">' + esc(d.description) + '</p>';
            }
            return html;
        },
    },
    {
        title: 'Trạng thái', field: 'status', width: 140, hozAlign: 'center', sorter: 'string',
        formatter(cell) {
            const d = cell.getRow().getData();
    ...
    {
        title: 'Số shot', field: 'shots_count', width: 100, hozAlign: 'center', sorter: 'number',
    },
    {
    ...
    {
        title: 'Ngày tạo', field: 'created_at', width: 140, hozAlign: 'center', sorter: 'string',
    },
    {
        title: '', field: 'id', width: 100, hozAlign: 'center', headerSort: false, frozen: true,
        formatter(cell) {
            const d = cell.getRow().getData();
            return '<div class="flex items-center justify-center gap-1">'
                ...
                + ' onclick="window.aivsProjectDeleteConfirm(this.dataset.url, this.dataset.name)">'
...
window.aivsProjectDeleteConfirm = function (url, name) {
    pendingDeleteUrl = url;
    const nameEl = document.getElementById('aivsDeleteProjectName');
    if (nameEl) nameEl.textContent = '"' + name + '"';
    document.getElementById('aivsDeleteProjectModal')?.showModal();
};

document.addEventListener('DOMContentLoaded', () => {
    const confirmBtn = document.getElementById('aivsConfirmDeleteBtn');
    ...
    confirmBtn.addEventListener('click', async function () {
        ...
        try {
            ...
            if (res.ok || res.redirected) {
                document.getElementById('aivsDeleteProjectModal')?.close();
                window.aivsProjectTable?.replaceData();
            } else {
                const data = await res.json().catch(() => ({}));
                alert(data.message || 'Xoá thất bại. Vui lòng thử lại.');
            }
        } catch (e) {
            console.error('[aivideostudiotemplate] delete project failed', e);
        ...
        return {
            ...
            _setup() {
                ...
                tableInst = new window.Tabulator('#project-table', {
                    ...
                    ajaxError: (error) => console.error('[aivideostudiotemplate] API error', error),
                    ...
                    placeholder: '<div class="py-16 text-center opacity-40">'
                        ...
                        + '<p class="text-sm">Chưa có project nào</p></div>',
                });

                window.aivsProjectTable = tableInst;
            },
