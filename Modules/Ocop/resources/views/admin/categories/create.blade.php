...
@section('title', 'Thêm danh mục OCOP')
...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-base-content">Thêm danh mục OCOP</h1>
    </div>
...
<form method="POST" action="{{ route('backend.ocop.categories.store') }}" novalidate
    ...
    <div class="card-body p-5 space-y-4">
        ...
        <div class="form-control">
            ...
            <input type="text" name="name" value="{{ old('name') }}"
                   ...
                   placeholder="VD: Đồ uống" maxlength="150" autofocus>
        ...
        <div class="form-control">
            <label class="label py-0 pb-1.5">
                ...
                <span class="label-text-alt text-xs text-base-content/40">Tên icon (tuỳ chọn)</span>
            </label>
            <input type="text" name="icon" value="{{ old('icon') }}"
                   class="input input-bordered input-sm w-full @error('icon') input-error @enderror"
                   maxlength="80" placeholder="VD: package">
        ...
        <label class="flex items-start gap-2.5 cursor-pointer select-none group">
            ...
            <input type="checkbox" name="is_active" value="1"
                   class="checkbox checkbox-sm checkbox-primary mt-0.5 shrink-0" {{ old('is_active', true) ? 'checked' : '' }}>
        ...
        <div class="flex gap-2 pt-2">
            ...
            <button type="submit" class="btn btn-primary btn-sm flex-1">Tạo danh mục</button>
        </div>
    </div>
</form>
</div>
