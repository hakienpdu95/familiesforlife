...
<div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            ...
            <p class="text-sm text-base-content/50 mt-0.5">Ngành hàng dùng để phân loại sản phẩm OCOP</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('backend.ocop.products.index') }}" class="btn btn-ghost btn-sm">← Sản phẩm OCOP</a>
            @can('create', \Modules\Ocop\Models\OcopCategory::class)
            <a href="{{ route('backend.ocop.categories.create') }}" class="btn btn-primary btn-sm gap-1.5">
    ...
    <form method="GET" class="flex flex-wrap gap-2 mb-5">
        ...
        @if(request('q'))
        <a href="{{ route('backend.ocop.categories.index') }}" class="btn btn-sm btn-ghost">Xoá lọc</a>
        @endif
    </form>
            ...
            <table class="table table-sm">
                ...
                <tr class="hover">
                    <td>
                        ...
                        <div class="text-xs text-base-content/40 font-mono ml-0">{{ $cat->slug }}</div>
                    </td>
                    ...
                    <td>
                        <div class="flex gap-1">
                            @can('update', $cat)
                            <a href="{{ route('backend.ocop.categories.edit', $cat) }}" class="btn btn-ghost btn-xs btn-square" title="Sửa">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @endcan
                            @can('delete', $cat)
                            <form method="POST" action="{{ route('backend.ocop.categories.destroy', $cat) }}"
