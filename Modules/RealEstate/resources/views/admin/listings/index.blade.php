...
@section('content')
<div x-data="realEstateListPage({{ Js::from([
    'apiUrl' => route('backend.api.real-estate.listings'),
]) }})">
        ...
        <div class="card-body py-3 px-4">
            <div class="flex flex-wrap gap-3 items-end">
                ...
                <div class="form-control w-40">
                    ...
                    <select x-model="filters.listing_type" @change="onFilterChange()" class="select select-sm select-bordered w-full">
                        <option value="">— Tất cả —</option>
                        @foreach(\Modules\RealEstate\Enums\ListingType::cases() as $lt)
                ...
                <div class="form-control w-48">
                    ...
                    <select x-model="filters.property_type" @change="onFilterChange()" class="select select-sm select-bordered w-full">
                ...
                <div class="form-control w-44">
                    <label class="label py-0.5"><span class="label-text text-xs font-medium">Trạng thái duyệt</span></label>
                    <select x-model="filters.approval_status" @change="onFilterChange()" class="select select-sm select-bordered w-full">
                        <option value="">— Tất cả —</option>
                        @foreach(\Modules\Approval\Enums\ApprovalStatus::cases() as $s)
    ...
    <div class="card bg-base-100 shadow-sm border border-base-200">
        <div class="card-body p-0 overflow-hidden tabulator-daisy">
            <div id="real-estate-listing-table"></div>
        </div>
    </div>
...
<dialog id="realEstateDeleteModal" class="modal">
    <div class="modal-box max-w-sm">
        <h3 class="font-bold text-lg text-error">Xác nhận xoá</h3>
        <p class="py-3 text-sm text-base-content/70">
            Bạn có chắc muốn xoá tin
            <strong id="realEstateDeleteItemTitle" class="text-base-content"></strong>?
        </p>
        <div class="modal-action mt-4">
            <button id="realEstateConfirmDeleteBtn" class="btn btn-error btn-sm">Xoá</button>
            <button class="btn btn-ghost btn-sm" onclick="realEstateDeleteModal.close()">Hủy</button>
        </div>
    </div>
