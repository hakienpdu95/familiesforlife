 ...
 * Alpine component + Tabulator (dataTree) cho dashboard/posts/categories — cùng pattern
 * article-index.js/organization-index.js nhưng KHÔNG phân trang remote: cây danh mục luôn tải
 * toàn bộ 1 lượt (CategoryApiController trả cây lồng nhau qua field "children", hoặc danh sách
 * phẳng khi có tìm kiếm — Tabulator tự hiển thị tree/phẳng tương ứng, không cần toggle JS).
 *
...
const COLUMNS = [
    ...
    {
        title: 'Số bài viết', field: 'articles_count', width: 120, hozAlign: 'center', sorter: 'number',
    },
    {
...
window.categoryDeleteConfirm = function (url, name) {
    ...
    const nameEl = document.getElementById('categoryDeleteItemName');
    if (nameEl) nameEl.textContent = '"' + name + '"';
    document.getElementById('categoryDeleteModal')?.showModal();
};

document.addEventListener('DOMContentLoaded', () => {
    const confirmBtn = document.getElementById('categoryConfirmDeleteBtn');
    ...
    confirmBtn.addEventListener('click', async function () {
        ...
        try {
            ...
            if (res.ok) {
                document.getElementById('categoryDeleteModal')?.close();
                window.categoryTable?.replaceData();
            } else {
                alert(data.message || 'Xoá thất bại. Vui lòng thử lại.');
            }
        } catch (e) {
            console.error('[post-category] delete failed', e);
        ...
        return {
            ...
            _setup() {
                ...
                tableInst = new window.Tabulator('#category-table', {
                    ...
                    ajaxError: (error) => console.error('[post-category] API error', error),
