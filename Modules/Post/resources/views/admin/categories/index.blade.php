...
<div x-data="categoryListPage({{ Js::from([
    'apiUrl' => route('backend.api.post.categories'),
]) }})">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-bold text-base-content">Danh mục bài viết</h1>
            <p class="text-sm text-base-content/50 mt-0.5">Cây danh mục dùng để phân loại bài viết</p>
        </div>
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('backend.post.categories.create') }}" class="btn btn-primary btn-sm gap-1.5">
...
<dialog id="categoryDeleteModal" class="modal">
    <div class="modal-box max-w-sm">
        <h3 class="font-bold text-lg text-error">Xác nhận xoá</h3>
        <p class="py-3 text-sm text-base-content/70">
            Bạn có chắc muốn xoá danh mục
            <strong id="categoryDeleteItemName" class="text-base-content"></strong>?
        </p>
        <div class="modal-action mt-4">
            <button id="categoryConfirmDeleteBtn" class="btn btn-error btn-sm">Xoá</button>
            <button class="btn btn-ghost btn-sm" onclick="categoryDeleteModal.close()">Hủy</button>
        </div>
    </div>
