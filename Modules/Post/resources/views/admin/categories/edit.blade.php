...
<form method="POST" action="{{ route('backend.post.categories.update', $category) }}" novalidate data-post-category-form>
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 xl:grid-cols-[1fr_268px] gap-6 items-start">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div class="form-control">
                                ...
                                <div class="color-picker-combo">
                                    <input type="color" id="colorPicker" value="{{ old('color_hex', $category->color_hex ?? '#94a3b8') }}">
                                    <input type="text" name="color_hex" id="colorText"
                                           value="{{ old('color_hex', $category->color_hex) }}"
                                           maxlength="7"
        ...
        <div class="xl:sticky xl:top-4 space-y-4">
            ...
            @php($articlesCount = $category->articles()->count())
            @if($articlesCount > 0)
            <div class="alert alert-info text-xs">
                <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                </svg>
                <span>Danh mục đang chứa <strong>{{ $articlesCount }}</strong> bài viết.</span>
            </div>
            @endif
