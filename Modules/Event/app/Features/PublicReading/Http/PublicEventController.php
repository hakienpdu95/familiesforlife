...
class PublicEventController extends Controller
{
    ...
    public function show(string $slug, EventStructuredDataBuilder $structuredDataBuilder): View
    {
        ...
        // HeritageSite::published(), KHÔNG find() thẳng: nullOnDelete chỉ tự dọn heritage_site_id
        // khi di tích bị xoá CỨNG, còn xoá mềm/chuyển về draft vẫn để lại giá trị cũ ở cột này.
        $heritageSite = $event->heritage_site_id
            ? HeritageSite::published()->find($event->heritage_site_id)
            : null;

        return view('event::public.show', compact('event', 'canonicalUrl', 'structuredData', 'heritageSite'));
    }
}
