...
<div>
    ...
    <form method="GET" class="flex flex-wrap gap-2 mb-5">
        ...
        @if(request('q'))
        <a href="{{ route('backend.event.categories.index') }}" class="btn btn-sm btn-ghost">Xoá lọc</a>
        @endif
    </form>
            ...
            <table class="table table-sm">
                <thead class="bg-base-200/60 text-xs uppercase tracking-wide">
                    <tr>
                        <th>Tên danh mục</th>
                        <th>Danh mục cha</th>
                        <th class="text-center">Số sự kiện</th>
                ...
                <tr class="hover">
                    ...
                    <td class="text-center text-sm">{{ $cat->events_count }}</td>
                    ...
                    <td>
                        <div class="flex gap-1">
                            @can('update', $cat)
                            <a href="{{ route('backend.event.categories.edit', $cat) }}" class="btn btn-ghost btn-xs btn-square" title="Sửa">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @endcan
                            @can('delete', $cat)
                            <form method="POST" action="{{ route('backend.event.categories.destroy', $cat) }}"
