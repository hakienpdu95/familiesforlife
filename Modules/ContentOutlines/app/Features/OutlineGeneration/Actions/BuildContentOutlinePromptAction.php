...
class BuildContentOutlinePromptAction
{
    ...
    private function buildTop(ContentOutlineInputData $input, ?CategoryContentFoundation $foundation, string $depth): string
    {
        ...
        // §4.28 (v1.26) — audience-first content brief: khác `target_audience` (mô tả TĨNH ai là
        // độc giả), 2 dòng này neo prompt vào NHIỆM VỤ CỤ THỂ + TRẠNG THÁI CẢM XÚC hiện tại của họ —
        // dùng ở Bước "Xác nhận ý định tìm kiếm" (Bản đồ Thấu cảm) + neo USP/CTA theo JTBD bên dưới.
        if (! empty($input->job_to_be_done)) {
            $rows[] = "- **Nhiệm vụ cần giải quyết (Job-to-be-done):** {$input->job_to_be_done}";
        }

        if (! empty($input->reader_emotional_state)) {
            $rows[] = "- **Cảm xúc/tâm trạng hiện tại của độc giả:** {$input->reader_emotional_state}";
        }
     ...
     * không mở lại.
     *
     * §4.28 (v1.26, framework "audience-first content brief" người dùng cung cấp trực tiếp, không
     * kèm URL) — nguồn đề xuất 1 framework 4 phần (Bản đồ Thấu cảm/Information Gain/Action-Driven
     * Outline/Điểm chạm Thương mại Thấu cảm) thay thế hoàn toàn cách tiếp cận từ khoá. Đối chiếu: 3/4
     * phần ĐÃ có cơ chế tương đương — Information Gain ≈ USP + differentiation note (§4.8/§4.12);
     * Action-Driven Outline (BLUF) ≈ answer-first (§4.11); Điểm chạm Thương mại ≈ bước CTA/`cta_url`
     * (§4.10/§4.18) — KHÔNG tạo 3 bước/mục output mới trùng lặp, chỉ MỞ RỘNG NỘI DUNG 3 cơ chế này
     * theo JTBD khi có (xem bên dưới). 1 phần THẬT còn thiếu: Bản đồ Thấu cảm (nỗi lo/điểm mù/giọng
     * điệu kỳ vọng) — bước "Xác nhận ý định tìm kiếm" hiện tại (§4.12, 3 câu hỏi a/b/c) chỉ khung theo
     * NHIỆM VỤ, chưa khung theo CẢM XÚC.
     *
     * Áp dụng, không đổi signature `handle()`/`buildBottom()`: (1) 2 field mới `job_to_be_done`/
     * `reader_emotional_state` (TOP, xem `buildTop()`); (2) bước "Xác nhận ý định tìm kiếm" (CẢ 3
     * depth) mở rộng thêm 3 câu hỏi (d)(e)(f) — nỗi lo lớn nhất/điểm mù chưa biết/giọng điệu kỳ vọng,
     * dùng Persona/JTBD/Cảm xúc ở TOP nếu có, tự suy luận từ chủ đề+từ khoá nếu không; `standard`/
     * `detailed` xuất thêm mục `## Bản đồ Thấu cảm` riêng (đủ dài để tách mục output); `brief` GỘP
     * vào câu trả lời chung của Bước 2 (không thêm mục output — giữ đúng tinh thần rút gọn §4.1); (3)
     * bước USP (`standard`/`detailed`) thêm 1 câu neo theo JTBD nếu có — đây CHÍNH LÀ information
     * gain, không phải mục mới; (4) bước dựng H2/H3 (CẢ 3 depth) thêm yêu cầu MỖI H2 kết bằng 1 câu
     * "Hành động cụ thể" (độc giả cần LÀM gì ngay sau khi đọc xong); (5) bước CTA (CẢ 3 depth) thêm 1
     * câu: nếu có JTBD, ưu tiên đặt CTA ngay sau H2 giải quyết TRỰC TIẾP nhiệm vụ đó, nêu rõ tên H2 đề
     * xuất, thay vì mặc định để cuối bài.
     *
     * KHÔNG áp dụng nguyên văn: 4 mục output riêng biệt của nguồn (Empathy Map/Information
     * Gain/Action-Driven Outline/Điểm chạm Thương mại) như 1 khối tách rời — sẽ tạo 2 cơ chế song
     * song cùng làm 1 việc (USP/answer-first/CTA đã làm 3/4 việc đó), đi ngược nguyên tắc chống trùng
     * lặp đã áp dụng xuyên suốt module (vd. §4.21 từ chối kiến trúc multi-step khi đã có cơ chế
     * tương đương).
     *
...
2. **Xác nhận ý định tìm kiếm + USP + đề xuất 1-2 phương án tiêu đề (H1)** — USP: 1 câu CỤ THỂ vì sao đọc bài NÀY thay vì các bài đã research ở Bước 1 (không chỉ nói "chất lượng hơn"); nếu có Job-to-be-done, khung USP theo hướng giúp hoàn thành nhiệm vụ đó NHANH/DỄ/ÍT RỦI RO hơn; thêm 1 câu ngắn nêu nỗi lo/điểm mù LỚN NHẤT của độc giả về chủ đề này ngay lúc này (dùng Persona/Job-to-be-done/Cảm xúc ở trên nếu có, tự suy luận từ chủ đề+từ khoá nếu không); tiêu đề chứa từ khoá mục tiêu ĐẶT GẦN ĐẦU tiêu đề và phản ánh đúng USP; đánh dấu rõ phương án MẠNH NHẤT (dự đoán CTR cao nhất) + 1 câu lý do.
...
4. **Dựng cấu trúc H2** — chọn 1 kiểu trình tự phù hợp chủ đề (từng bước theo thời gian / giải quyết vấn đề / tổng quát → cụ thể) rồi giữ nhất quán; mỗi heading trả lời 1 câu hỏi thật; mỗi H2 nên MỞ ĐẦU bằng 1 câu trả lời TRỰC TIẾP câu hỏi của chính heading đó rồi mới giải thích thêm (answer-first — dễ được trích dẫn bởi featured snippet/AI answer engine; nếu Bước 1 quan sát được featured snippet đang hiển thị dạng danh sách/bảng cho câu hỏi tương ứng, format câu trả lời mở đầu ĐÚNG dạng đó thay vì đoạn văn); mỗi H2 nên có 2-3 điểm hỗ trợ VÀ kết bằng 1 câu "Hành động cụ thể" nêu rõ độc giả cần LÀM gì ngay sau khi đọc xong phần đó; các H2 cùng cấp dùng CÙNG dạng ngữ pháp (VD tất cả là câu hỏi, hoặc tất cả bắt đầu bằng động từ) để dễ theo dõi; Dàn ý nên khép lại bằng 1 H2 "Kết luận" ngắn tóm lại luận điểm chính (không lặp nguyên văn), trước khi chuyển sang FAQ.
...
6. **CTA ngắn** — 1 câu gợi ý bước tiếp theo phù hợp mục tiêu bài viết/ý định tìm kiếm; nếu có Job-to-be-done, ưu tiên đặt CTA ngay sau phần nội dung giải quyết TRỰC TIẾP nhiệm vụ đó thay vì mặc định cuối bài — nêu rõ vị trí (H2 nào) đề xuất; nếu có "CTA URL" ở trên, viết CTA dưới dạng 1 câu chuyển tiếp TỰ NHIÊN mời truy cập ĐÚNG URL đó; nếu ý định là thông tin (informational), ưu tiên gợi ý đọc thêm/hướng dẫn tiếp theo, KHÔNG chèn CTA bán hàng.
...
2. **Xác nhận ý định tìm kiếm + Bản đồ Thấu cảm** — nêu rõ đây là bài học cái mới / so sánh lựa chọn / giải quyết vấn đề cụ thể / hướng dẫn từng bước; nếu đã cho ở trên, xác nhận hoặc điều chỉnh có giải thích; viết thêm 1 đoạn ngắn trả lời: (a) đọc xong bài, độc giả cần LÀM/ĐẠT được gì, (b) độc giả biết gì lúc BẮT ĐẦU đọc và cần biết gì lúc KẾT THÚC, (c) điều gì khiến họ bỏ bài này đi tìm bài khác; sau đó trả lời thêm 3 câu cho Bản đồ Thấu cảm: (d) độc giả đang hoang mang/lo lắng NHẤT điều gì về chủ đề này ngay lúc này, (e) 1 ĐIỂM MÙ họ chưa biết nhưng thực sự cần biết trước khi hành động, (f) giọng điệu họ mong đợi ở bài viết (an ủi/dứt khoát/chuyên môn/đồng cảm...) — dùng Persona/Job-to-be-done/Cảm xúc ở trên nếu có, tự suy luận từ chủ đề+từ khoá nếu không được cho; dùng CẢ 2 đoạn này để soi lại từng H2 phía dưới có thực sự phục vụ ý định VÀ chạm đúng nỗi lo hay không.
...
4. **Xác định USP** — 1-2 câu CỤ THỂ vì sao đọc bài NÀY thay vì các bài đã research ở Bước 1 (VD nguồn/dữ liệu mới hơn, góc nhìn khác, hướng dẫn chi tiết hơn) — không chỉ nói chung "chất lượng hơn"; nếu có Job-to-be-done, khung USP theo hướng giúp hoàn thành nhiệm vụ đó NHANH/DỄ/ÍT RỦI RO hơn.
...
12. **Gợi ý CTA/bước tiếp theo** — 1 CTA cụ thể phù hợp mục tiêu bài viết/ý định tìm kiếm đã cho ở trên (VD đọc thêm bài liên quan, dùng công cụ/máy tính, đăng ký nhận tư vấn, liên hệ...); nếu có Job-to-be-done, ưu tiên đặt gợi ý CTA ngay sau H2 nào giải quyết TRỰC TIẾP nhiệm vụ đó (không nhất thiết phải ở cuối bài) — nêu rõ tên H2 đề xuất; nếu có "CTA URL" ở trên, viết CTA dưới dạng 1 câu chuyển tiếp TỰ NHIÊN mời truy cập ĐÚNG URL đó (không chỉ dán trơ URL); nếu ý định tìm kiếm là thông tin (informational), ưu tiên CTA hướng dẫn tiếp theo, KHÔNG chèn CTA bán hàng cứng nhắc.
...
3. **Xác nhận ý định tìm kiếm + Bản đồ Thấu cảm** — nêu rõ đây là bài học cái mới / so sánh lựa chọn / giải quyết vấn đề cụ thể / hướng dẫn từng bước; nếu đã cho ở trên, xác nhận hoặc điều chỉnh có giải thích; viết thêm 1 đoạn ngắn trả lời: (a) đọc xong bài, độc giả cần LÀM/ĐẠT được gì, (b) độc giả biết gì lúc BẮT ĐẦU đọc và cần biết gì lúc KẾT THÚC, (c) điều gì khiến họ bỏ bài này đi tìm bài khác; sau đó trả lời thêm 3 câu cho Bản đồ Thấu cảm: (d) độc giả đang hoang mang/lo lắng NHẤT điều gì về chủ đề này ngay lúc này, (e) 1 ĐIỂM MÙ họ chưa biết nhưng thực sự cần biết trước khi hành động, (f) giọng điệu họ mong đợi ở bài viết (an ủi/dứt khoát/chuyên môn/đồng cảm...) — dùng Persona/Job-to-be-done/Cảm xúc ở trên nếu có, tự suy luận từ chủ đề+từ khoá nếu không được cho; dùng CẢ 2 đoạn này để soi lại từng H2 phía dưới có thực sự phục vụ ý định VÀ chạm đúng nỗi lo hay không.
...
5. **Xác định USP** — 1-2 câu CỤ THỂ vì sao đọc bài NÀY thay vì các bài đã research/đánh giá ở Bước 1-2 (VD nguồn/dữ liệu mới hơn, góc nhìn khác, hướng dẫn chi tiết hơn) — không chỉ nói chung "chất lượng hơn"; nếu có Job-to-be-done, khung USP theo hướng giúp hoàn thành nhiệm vụ đó NHANH/DỄ/ÍT RỦI RO hơn.
