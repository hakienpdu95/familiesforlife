...
@section('title', 'Di tích/Di sản')

@section('content')
<div x-data="heritageSiteListPage({{ Js::from([
    'apiUrl' => route('backend.api.heritage.sites'),
]) }})">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div>
            <h1 class="text-2xl font-bold text-base-content">Di tích/Di sản</h1>
        ...
        <div class="flex items-center gap-2">
            @can('create', \Modules\Heritage\Models\HeritageSite::class)
            <a href="{{ route('backend.heritage.sites.create') }}" class="btn btn-primary btn-sm gap-1.5">
                ...
                <div class="form-control flex-1 min-w-52">
                    ...
                    <div class="input input-sm input-bordered flex items-center gap-2 bg-base-100">
                        ...
                        <input type="text" x-model="filters.search" @input.debounce.350ms="onFilterChange()"
                               placeholder="Nhập tên di tích..." class="grow bg-transparent outline-none text-sm">
    ...
    <div class="card bg-base-100 shadow-sm border border-base-200">
        <div class="card-body p-0 overflow-hidden tabulator-daisy">
            <div id="heritage-site-table"></div>
        </div>
    </div>
...
<dialog id="heritageSiteDeleteModal" class="modal">
    <div class="modal-box max-w-sm">
        <h3 class="font-bold text-lg text-error">Xác nhận xoá</h3>
        <p class="py-3 text-sm text-base-content/70">
            Bạn có chắc muốn xoá di tích
            <strong id="heritageSiteDeleteItemName" class="text-base-content"></strong>?
        </p>
        <div class="modal-action mt-4">
            <button id="heritageSiteConfirmDeleteBtn" class="btn btn-error btn-sm">Xoá</button>
            <button class="btn btn-ghost btn-sm" onclick="heritageSiteDeleteModal.close()">Hủy</button>
        </div>
    </div>
...
@push('styles')
    <x-tabulator-theme />
    @vite(['Modules/Heritage/resources/assets/sass/heritage.scss'], 'build/backend')
@endpush
