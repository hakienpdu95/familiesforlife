/**
 * pages/heritage-site-index.js
 * Alpine component + Tabulator cho dashboard/heritage/sites — mirror
 * Modules/Ocop/resources/assets/js/pages/ocop-product-index.js: remote pagination/sort/filter
 * qua API, xoá qua AJAX + modal xác nhận.
 *
 * Server data truyền vào qua x-data="heritageSiteListPage({{ Js::from([...]) }})".
 */
...
const COLUMNS = [
    ...
    {
        title: 'Di tích', field: 'name', minWidth: 220, sorter: 'string',
        formatter(cell) {
    ...
    {
        title: 'Loại hình', field: 'heritage_type_label', minWidth: 160, headerSort: false,
        formatter(cell) { return esc(cell.getValue()); },
    },
    {
        title: 'Xếp hạng', field: 'rank_label', minWidth: 150, headerSort: false,
        formatter(cell) { return esc(cell.getValue()); },
    },
    {
        ...
        formatter(cell) {
            ...
            if (d.can_delete) {
                html += '<button class="btn btn-ghost btn-xs btn-square text-error" title="Xoá"'
                    ...
                    + ' onclick="window.heritageSiteDeleteConfirm(this.dataset.url, this.dataset.name)">'
...
window.heritageSiteDeleteConfirm = function (url, name) {
    pendingDeleteUrl = url;
    const nameEl = document.getElementById('heritageSiteDeleteItemName');
    if (nameEl) nameEl.textContent = '"' + name + '"';
    document.getElementById('heritageSiteDeleteModal')?.showModal();
};

document.addEventListener('DOMContentLoaded', () => {
    const confirmBtn = document.getElementById('heritageSiteConfirmDeleteBtn');
    ...
    confirmBtn.addEventListener('click', async function () {
        ...
        try {
            ...
            if (res.ok) {
                document.getElementById('heritageSiteDeleteModal')?.close();
                window.heritageSiteTable?.replaceData();
            } else {
                alert(data.message || 'Xoá thất bại. Vui lòng thử lại.');
            }
        } catch (e) {
            console.error('[heritage-site] delete failed', e);
...
document.addEventListener('alpine:init', () => {
    Alpine.data('heritageSiteListPage', (serverData = {}) => {
        ...
        return {
            ...
            _setup() {
                ...
                tableInst = new window.Tabulator('#heritage-site-table', {
                    ajaxURL:    apiUrl,
                    ...
                    ajaxError: (error) => console.error('[heritage-site] API error', error),
                    ...
                    placeholder: '<div class="py-16 text-center opacity-40">'
                        ...
                        + '<p class="text-sm">Chưa có di tích nào</p></div>',
                });

                window.heritageSiteTable = tableInst;
            },
