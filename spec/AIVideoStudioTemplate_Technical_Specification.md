...
- Quy trình 3 giai đoạn "Xây dựng nền móng / Nghiên cứu & Lập cấu trúc / Viết & Tinh chỉnh Kịch bản" (v1.31, do người dùng cung cấp trực tiếp trong hội thoại, tóm tắt từ 1 video hướng dẫn viết kịch bản viral, không phải file riêng trong `spec/`) — 14 mục across 3 giai đoạn. Rà đủ 14 mục, chỉ 6 mục đúng vai trò BƯỚC 2 "viết kịch bản cho topic/công thức đã chọn sẵn" — xem docblock `BuildMasterScriptPromptAction` (§13.6, v1.31) cho lý do đầy đủ từng mục áp dụng/loại.
...
- **v1.31 — 2 test mới (19 Unit test tổng, cùng file trên)**: chuỗi trả về LUÔN chứa "VẤN ĐỀ BỀ MẶT"/"VẤN ĐỀ SÂU XA", "ĐANG Ở ĐÂU lúc mở đầu"/"THAY ĐỔI NHẬN THỨC NHƯ THẾ NÀO", "ĐỘ PHỨC TẠP TĂNG DẦN", "ĐẶT CÂU HỎI/GỢI MỞ", "NHƯNG...VÌ VẬY", "câu NGẮN, câu TRUNG BÌNH và câu DÀI" — bất kể `isMotherBaby`; chuỗi trả về KHÔNG BAO GIỜ chứa "Hero"/"Vũ trụ Kể chuyện"/"Universe"/"Roasting"/"roast"/"Freewriting"/"đồng hồ hẹn giờ" (cụm khoá của phần nguồn CỐ Ý không áp dụng, §13.2) — test fail nếu sau này ai copy nhầm nguyên văn phần bị loại vào Action.
