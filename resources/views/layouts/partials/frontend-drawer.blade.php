...
<div class="drawer-side z-50">
    ...
    <div class="menu bg-base-100 min-h-full w-80 p-4 text-base-content">
        <a href="{{ route('post.public.home', ['locale' => $locale ?? config('post.default_locale')]) }}" class="flex items-center gap-2 mb-6 px-1">
        ...
        <ul class="flex flex-col gap-1">
            ...
            <li>
                ...
                <ul x-show="mobileSub === {{ $loop->index }}" x-transition x-cloak class="pl-2">
                    <li><a href="{{ route('post.public.category', ['locale' => $locale, 'category' => $cat->slug]) }}">Tất cả {{ $cat->name }}</a></li>
                    @foreach($cat->children as $child)
                    <li><a href="{{ route('post.public.category', ['locale' => $locale, 'category' => $child->slug]) }}">{{ $child->name }}</a></li>
                    @endforeach
                </ul>
                @else
                <a href="{{ route('post.public.category', ['locale' => $locale, 'category' => $cat->slug]) }}" class="font-semibold">{{ $cat->name }}</a>
                @endif
            </li>
            @endforeach
        </ul>
